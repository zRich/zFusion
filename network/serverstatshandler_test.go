...
import (
	...
	"github.com/zRich/zFusion/common/metrics"
	"github.com/zRich/zFusion/common/metrics/metricsfakes"
...
func TestConnMetricsGRPCServer(t *testing.T) {
	...
	srv, err := network.NewGRPCServerFromListener(
		...
		network.ServerConfig{
			SecOpts:            network.SecureOptions{UseTLS: false},
			ServerStatsHandler: network.NewServerStatsHandler(fakeProvider),
		},
	)
	gt.Expect(err).NotTo(HaveOccurred())
