...
import (
	...
	"github.com/zRich/zFusion/common/flogging/floggingtest"
...
func TestCreds(t *testing.T) {
	...
	config := network.NewTLSConfig(tlsConfig)

	logger, recorder := floggingtest.NewTestLogger(t)

	creds := network.NewServerTransportCredentials(config, logger)
	_, _, err = creds.ClientHandshake(context.Background(), "", nil)
	require.EqualError(t, err, network.ErrClientHandshakeNotImplemented.Error())
	err = creds.OverrideServerName("")
	require.EqualError(t, err, network.ErrOverrideHostnameNotSupported.Error())
...
func TestConfig(t *testing.T) {
	t.Parallel()
	config := network.NewTLSConfig(&tls.Config{
		ServerName: "bueno",
	})
...
func TestAddRootCA(t *testing.T) {
	...
	config := network.NewTLSConfig(&tls.Config{ClientCAs: certPool})
