...
import (
	...
	"github.com/zRich/zFusion/common/logging"
...
var (
	...
	logger = logging.GetLogger("did") //nolint:gochecknoglobals
)
...
// Document DID Document definition.
...
func populateVerificationRelationships(doc *Document, raw *rawDoc) error {
...
func populateProofs(context, didID, baseURI string, rawProofs []interface{}) ([]Proof, error) {
	...
	for _, rawProof := range rawProofs {
		...
		proofValue, err := DecodeProofValue(stringEntry(emap[proofKey]), stringEntry(emap[jsonldType]))
...
func populateVerification(doc *Document, rawVerification []interface{},
...
func getVerification(doc *Document, rawVerification interface{},
	relationship VerificationRelationship) ([]Verification, error) {
...
// JSONBytes converts document to json bytes.
func (doc *Document) JSONBytes() ([]byte, error) {
...
func contextWithBase(doc *Document) Context {
...
// nolint:gocyclo
func (doc *Document) VerificationMethods(customVerificationRelationships ...VerificationRelationship) map[VerificationRelationship][]Verification { //nolint:lll
...
func populateRawProofs(context, didID, baseURI string, proofs []Proof) []interface{} {
	...
	for _, p := range proofs {
		...
		rawProofs = append(rawProofs, map[string]interface{}{
			...
			k:                  EncodeProofValue(p.ProofValue, p.Type),
...
func BuildDoc(opts ...DocOption) *Document {
	doc := &Document{}
